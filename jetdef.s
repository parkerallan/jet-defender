...
.include "jetdef-sfx.s"
.include "jetdef-music.s"

.segment "DPCM"
.incbin "jetdef-music.dmc"
...
;*****************************************************************
 ...
 .proc main
   ...
   lda #1 ; NTSC 
   ldx #.lobyte(music_data_jetdef)
   ldy #.hibyte(music_data_jetdef) 
   jsr famistudio_init
...
@notSmartBomb:
	...
	clc
	adc #1 ; A will be 1 (enemy ship) or 2 (missiles)
...
@noanimate:
	...
	lda player_dead
	cmp #0 ; check if the player is not currently dead
	bne @notlevelwithplayer
	
	lda oam ; get the player's Y position
	clc
	adc #8 ; add on the height of the player
	lda oam,x ; get enemy Y position
	clc
	adc enemydata+8,y ; add on the enemy's height
	cmp oam ; compare with the player's Y position
	...
	cmp oam+3 ; is the enemies X plus it's width smaller than the player's Y position
	bcc @notlevelwithplayer

	lda oam ; get the players Y position	
	clc
	adc #12 ; add on the width of the player
	cmp oam,x ; is the enemies Y larger than the player plus it's width?
	bcc @notlevelwithplayer

	lda oam,x ; get the enemy Y position
	clc
	adc enemydata+7,y ; add on it's width
	cmp oam ; is the enemies Y plus it's width smaller than the player's Y position
	bcc @notlevelwithplayer
...
.proc display_highscore
	vram_set_address (NAME_TABLE_0_ADDRESS + 1 * 32 + 20)
...
not_gamepad_left:
	...
	beq not_gamepad_right
		...
		sta oam + 7
		sta oam + 15
not_gamepad_right: ;test
	lda gamepad
	and #PAD_U
	beq not_gamepad_up
		; gamepad has been pressed up
		lda oam ; get current y of ship
		sec
		sbc #24 ; allow with width of ship
		cmp #240
		beq not_gamepad_up
		lda oam ; get current y of ship
		sec
...
not_gamepad_up:
	...
	beq not_gamepad_down
		; gamepad has been pressed down
		lda oam ; get current y of ship
		clc
		adc #48 ; allow with width of ship
		cmp #240
		beq not_gamepad_down
		lda oam ; get current y of ship
		clc
...
not_gamepad_down:
	...
	lda oam ; get player Y position
	clc
	adc #12 ; adjust bullet position relative to ship's y position
	sta oam + 16 ; set bullet Y position
	lda #4
...
title_text:
.byte "J E T  D E F E N D E R",0

jet_tile:
.byte 094, 094, 094, 094, 094, 094, 094, 096, 094, 094, 094,0

jet_tile1:
.byte 094, 094, 097, 098, 099, 100, 101, 102, 103, 094, 094,0

jet_tile2:
.byte 094, 094, 104, 105, 106, 107, 108, 109, 110, 136, 094,0

jet_tile3:
.byte 094, 094, 111, 112, 113, 114, 115, 116, 117, 118, 135,0

jet_tile4:
.byte 094, 119, 120, 121, 122, 123, 124, 125, 126, 127, 128,0

jet_tile5:
.byte 129, 130, 131, 094, 094, 094, 132, 133, 134, 094, 094,0

press_play_text:
.byte "PRESS  START",0

game_credits_line:
.byte 016, 017, 018, 019, 020, 021, 022, 023,0
...
.proc display_title_screen
	...
	vram_set_address (NAME_TABLE_0_ADDRESS + 4 * 32 + 5);title text
	assign_16i text_address, title_text
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 10 * 32 + 10);jet graphic
	assign_16i text_address, jet_tile
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 11 * 32 + 10)
	assign_16i text_address, jet_tile1
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 12 * 32 + 10)
	assign_16i text_address, jet_tile2
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 13 * 32 + 10)
	assign_16i text_address, jet_tile3
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 14 * 32 + 10)
	assign_16i text_address, jet_tile4
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 15 * 32 + 10)
	assign_16i text_address, jet_tile5
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 21 * 32 + 9);start text
	assign_16i text_address, press_play_text
	jsr write_text

	vram_set_address (NAME_TABLE_0_ADDRESS + 25 * 32 + 11);credits
	assign_16i text_address, game_credits_line
	jsr write_text
	...
	vram_set_address (ATTRIBUTE_TABLE_0_ADDRESS + (8 * 5))
	assign_16i paddr, title_attributes
	...
	vram_set_address (ATTRIBUTE_TABLE_0_ADDRESS + (8 * 6))
	assign_16i paddr, title_attributes
...
.segment "RODATA"
enemy_source_data:
.byte 008,012,004,000,002,002,012,012,002 ; enemy ship
.byte 036,037,001,001,003,003,008,007,002 ; rocket
.byte 016,019,001,002,003,006,008,008,003 ; bomb
